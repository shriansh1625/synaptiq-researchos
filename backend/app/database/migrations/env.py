"""Alembic migration environment for async SQLAlchemy + PostgreSQL."""
...
def get_database_url() -> str:
    """Resolve the async PostgreSQL URL from application settings."""
    return to_async_database_url(get_settings().database_url)


def run_migrations_offline() -> None:
    """Run migrations in 'offline' mode without a live DBAPI connection."""
    context.configure(
...
def do_run_migrations(connection: Connection) -> None:
    """Configure Alembic and run migrations against a connection."""
    context.configure(
